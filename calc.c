...
//0x30 = '0' character decimal value is 48 and similarly 0x41=A (70)
...
void lcd_init()
{
  LPC_PINCON->PINSEL1 &=0xFC003FFF;        //P0.23 - P0.28 io lines
 	LPC_GPIO0->FIOMASK = 0xE07FFFFF;         //mask all except P0.23 - P0.28 at CND
	LPC_GPIO0->FIODIR |= dt;								 //data lines 
	LPC_GPIO0->FIODIR |= rs;								 //register set
	LPC_GPIO0->FIODIR |= en;								 //enable
	clear_ports();
  delay(3200);
...
//=== INTERRUPT HANDLER ===
//when interrupt is raised the keyboard is initialized and the keyboard function starts
void  EINT3_IRQHandler(void)
{
	for(i=0;i<1000;i++);
    ...
    temp3 &= 0x000000F0;
        ...
        LPC_GPIO0->FIOMASK = 0xFFFFF0FF;        //setting rows to 1
			...
			if(check=='=')
			{
				...
				infix_to_postfix();
				eval_post();
				for(i=0;i<500000;i++); // delay added
				while(top!=-1)
				{
					expr[top]='\0';
					pop();
